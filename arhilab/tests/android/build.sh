...
"$JAVA_HOME/bin/javac" -encoding UTF-8 -source 8 -target 8 -cp "$AJAR" -d build/smoke/classes tests/android/Smoke.java
...
"$BT/apksigner" sign --ks build/debug.keystore --ks-key-alias androiddebugkey --ks-pass pass:android --out build/smoke/smoke.apk build/smoke/aligned.apk
