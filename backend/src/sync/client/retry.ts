...
export function classifyFailure(failure:Failure):'retry'|'conflict'|'auth_required'|'permanent' {
  ...
  if (failure.httpStatus===403 || failure.httpStatus===400 || failure.httpStatus===413) return 'permanent';
...
export function retryDelayMs(attempt:number,random:number,retryAfterSeconds?:number) {
  if (!Number.isInteger(attempt) || attempt<1 || !Number.isFinite(random) || random<0 || random>=1 || (retryAfterSeconds !== undefined && (!Number.isFinite(retryAfterSeconds) || retryAfterSeconds<0))) throw Error('Invalid retry parameters');
